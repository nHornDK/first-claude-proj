...
public class ItemsControllerTests : IDisposable
{
    ...
    {
        var item = new ItemEntity { Name = "Test Item", Description = "Desc" };

        var result = await _sut.Create(item);

        var created = Assert.IsType<CreatedAtActionResult>(result.Result);
        var returned = Assert.IsType<ItemEntity>(created.Value);
    ...
    {
        await _sut.Create(new ItemEntity { Name = "Test Item" });
    ...
    {
        var created = (CreatedAtActionResult)(await _sut.Create(new ItemEntity { Name = "Test" })).Result!;
        var id = ((ItemEntity)created.Value!).Id;
    ...
    {
        var created = (CreatedAtActionResult)(await _sut.Create(new ItemEntity { Name = "Original" })).Result!;
        var id = ((ItemEntity)created.Value!).Id;

        var result = await _sut.Update(id, new ItemEntity { Id = id, Name = "Updated", Description = "New desc" });
    ...
    public async Task Update_PersistsChanges()
    {
        ...
        await _sut.Update(id, new ItemEntity { Id = id, Name = "Updated", Description = "New desc" });
    ...
    {
        var created = (CreatedAtActionResult)(await _sut.Create(new ItemEntity { Name = "Original" })).Result!;
        var item = (ItemEntity)created.Value!;
        var originalCreatedAt = item.CreatedAt;

        await _sut.Update(item.Id, new ItemEntity { Id = item.Id, Name = "Updated", CreatedAt = DateTime.UtcNow.AddYears(1) });
    ...
    {
        var result = await _sut.Update(999, new ItemEntity { Id = 999, Name = "X" });
    ...
    {
        var result = await _sut.Update(1, new ItemEntity { Id = 99, Name = "X" });
