...
aws ecs update-service   --cluster dendo_cluster   --service dendo_service   --enable-execute-command   --region eu-north-1
...
TASK_ARN=$(aws ecs list-tasks \
  ...
  --query 'taskArns[0]' \
  --output text)
aws ecs update-service   --cluster dendo_cluster   --service dendo_service --task "$TASK_ARN"  --enable-execute-command   --region eu-north-1
